...
currentURL = () =>
	new URL(
		inIframe
			? /(https?:\/\/(?:[\d\w]+\.)?[\d\w\.]+(?::\d+)?)/g.exec(document.referrer)?.[0] || location.href
			: location.href
	);
...
if (dataSpecified) json = base64ToJson();
...
addEventListener('DOMContentLoaded', () => {
	...
	if (autoUpdateURL) document.body.classList.add('autoUpdateURL');
	if (autoParams) document.querySelector('.auto-params > input').checked = true;
	if (inIframe)
		// Remove menu options that don't work in iframe.
		for (const e of document.querySelectorAll('.no-frame')) e.remove();
	if (hideMenu) document.querySelector('.top-btn.menu').classList.add('hidden');
	if (hideEditor) {
	...
	document
		.querySelectorAll('.clickable > img')
	...
	const notif = document.querySelector('.notification'),
		...
		error = (msg, time) => {
			...
			time &&
				setTimeout(
					() =>
						(notif.animate({ opacity: '0', bottom: '-50px', offset: 1 }, { easing: 'ease', duration: 500 }).onfinish =
		...
		allGood = e => {
			...
			if (e.timestamp && new Date(e.timestamp).toString() === 'Invalid Date') {
				...
				// If a URL is found without a protocol
				...
				if (allowPlaceholders !== 2)
					(invalid = true),
						(err = `URL should have a protocol. Did you mean <span class="inline full short">http://${makeShort(
							re[2],
							30,
							600
						).replace(' ', '')}</span>?`);
		...
		markup = (txt, opts) => {
			if (opts.replaceEmojis)
				txt = txt.replace(/(?<!code(?: \w+=".+")?>[^>]+)(?<!\/[^\s"]+?):((?!\/)\w+):/g, (match, p) =>
	...
	buildGui = (object, opts) => {
		gui.innerHTML = `
            ...
            <div class="edit">
                <div class="linkName">
                    <div class="editIcon">
                        <span class="imgParent" ${
													object?.author?.icon_url
														? 'style="content: url(' + encodeHTML(object.author.icon_url) + ')"'
														: ''
												}></span>
                        <input class="editAuthorLink" type="text" value="${encodeHTML(
													object?.author?.icon_url || ''
												)}" placeholder="Ícono de autor" autocomplete="off"/>
                    </div>
                    <div class="editName">
                        <input class="editAuthorName" type="text" maxlength="256" value="${encodeHTML(
													object?.author?.name || ''
												)}" placeholder="Nombre de autor" autocomplete="off" />
                    </div>
                    <div class="editUrlAuthor">
                        <input class="editAuthorUrl" type="text" maxlength="256" value="${encodeHTML(
													object?.author?.url || ''
												)}" placeholder="URL de autor" autocomplete="off" />
                    </div>
                </div>
                <form method="post" enctype="multipart/form-data">
                    ...
                    <label for="file2">
						...
						<div class="item footer rows2"><p class="ttle">Título</p></div>
            <div class="edit" style="height: 35px;">
                <div class="linkName">
                    <div class="editName">
                        <input class="editTitle" placeholder="Título" maxlength="256" value="${encodeHTML(
													object?.title || ''
												)}" autocomplete="off" />
                    </div>
                    <div class="editName">
                        <input class="editTitleURL" type="text" maxlength="2048" value="${encodeHTML(
													object?.url || ''
												)}" placeholder="URL del título" autocomplete="off" />
                    </div>
                </div>
            </div>
            <div class="item description"><p class="ttle">Descripción</p></div>
            <div class="edit">
                <textarea class="editDescription" placeholder="Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi suscipit tincidunt est pellentesque faucibus." maxlength="4096" autocomplete="off">${encodeHTML(
									object?.description || ''
								)}</textarea>
            </div>
            <div class="item fields"><p class="ttle">Fields</p></div>
            <div class="edit"></div>
            <div class="item thumbnail largeImg"><p class="ttle">Miniatura</p></div>
            <div class="edit">
                <div class="linkName">
                    <div class="editIcon">
                        <span class="imgParent" ${
													object?.thumbnail?.url ? 'style="content: url(' + encodeHTML(object.thumbnail.url) + ')"' : ''
												}></span>
                        <div class="txtCol">
                            <input class="editThumbnailLink" type="text" value="${encodeHTML(
															object?.thumbnail?.url || ''
														)}" placeholder="URL de miniatura" autocomplete="off" />
                            <form method="post" enctype="multipart/form-data">
            ...
            <div class="item image largeImg"><p class="ttle">Imagen</p></div>
            <div class="edit">
                <div class="linkName">
                    <div class="editIcon">
                        <span class="imgParent" ${
													object?.image?.url ? 'style="content: url(' + encodeHTML(object.image.url) + ')"' : ''
												}></span>
                        <div class="txtCol">
                            <input class="editImageLink" type="text" value="${encodeHTML(
															object?.image?.url || ''
														)}" placeholder="URL de imagen" autocomplete="off" />
                            <form method="post" enctype="multipart/form-data">
            ...
            <div class="item footer rows2"><p class="ttle">Pie de página</p></div>
            <div class="edit">
                <div class="linkName">
                    <div class="editIcon">
                        <span class="imgParent" ${
													object?.footer?.icon_url
														? 'style="content: url(' + encodeHTML(object.footer.icon_url) + ')"'
														: ''
												}></span>
                        <input class="editFooterLink" type="text" value="${encodeHTML(
													object?.footer?.icon_url || ''
												)}" placeholder="Ícono del footer" autocomplete="off"/>
                    </div>
                    <div class="editName">
                        <input class="editFooterText" type="text" maxlength="2048" value="${encodeHTML(
													object?.footer?.text || ''
												)}" placeholder="Texto del footer" autocomplete="off" />
                    </div>
                </div>
                <form method="post" enctype="multipart/form-data">
		...
		const fieldsEditor = gui.querySelector('.fields ~ .edit'),
			addField = `
            <div class="addField">
                <p>New Field</p>
		...
		gui.querySelectorAll('.removeBtn').forEach(e => {
			e.addEventListener('click', el => {
				...
				if (field) {
					let i = Array.from(fields.children).indexOf(field),
						jsonField = object.fields[i];
					if (jsonField) {
						object.fields.splice(i, 1);
						field.remove();
						update(object);
					}
				}
		...
		document.querySelectorAll('.gui>.item:not(.fields)').forEach(e =>
		...
		titleURL = gui.querySelector('.editTitleURL');
		...
		gui.querySelectorAll('textarea, input').forEach(e =>
			e.addEventListener('input', el => {
				const value = el.target.value,
					field = el.target.closest('.field');
				if (field) {
					const jsonField =
						json.fields[
							Array.from(fields.children)
								.filter(e => e.className === 'field')
								.indexOf(field)
						];
					if (jsonField) {
					...
					switch (el.target) {
						...
						case titleURL:
							...
							break;
						case authorName:
							(json.author ??= {}), (json.author.name = value);
							break;
						case authorUrl:
							(json.author ??= {}), (json.author.url = value);
							break;
						case authorLink:
							(json.author ??= {}), (json.author.icon_url = value), imgSrc(el.target.previousElementSibling, value);
							break;
						case desc:
						...
						case thumbLink:
							(json.thumbnail ??= {}),
								(json.thumbnail.url = value),
								imgSrc(el.target.closest('.editIcon').querySelector('.imgParent'), value);
							break;
						case imgLink:
							(json.image ??= {}),
								(json.image.url = value),
								imgSrc(el.target.closest('.editIcon').querySelector('.imgParent'), value);
							break;
						case footerText:
							(json.footer ??= {}), (json.footer.text = value);
							break;
						case footerLink:
							(json.footer ??= {}), (json.footer.icon_url = value), imgSrc(el.target.previousElementSibling, value);
							break;
					}
				}
				update(json);
		...
		if (opts?.guiTabs) {
			let tabs = opts.guiTabs.split?.(/, */) || opts.guiTabs,
				bottomKeys = ['footer', 'image'],
				topKeys = ['author', 'content'];
			...
			Array.from(opts.activate)
				.map(el => el.className)
				.map(clss => '.' + clss.split(' ').slice(0, 2).join('.'))
				.forEach(clss => document.querySelectorAll(clss).forEach(e => e.classList.add('active')));
		} else document.querySelectorAll('.item.author, .item.description').forEach(clss => clss.classList.add('active'));

		if (opts?.newField) {
			let last = fields.children[fields.childElementCount - 2],
				el = last.querySelector('.designerFieldName > input');
		...
		let upload = form => {
			...
			fetch('https://tempfile.site/api/files', {
				...
				.then(res => {
					...
					imgSrc(
						form.previousElementSibling.querySelector('.editIcon > .imgParent') ||
							form.closest('.editIcon').querySelector('.imgParent'),
						res.link
					);
					let input = form.previousElementSibling.querySelector('.editIcon > input') || form.previousElementSibling;
					input.value = res.link;
					if (input === authorLink) ((json ??= {}).author ??= {}).icon_url = res.link;
					else if (input === thumbLink) ((json ??= {}).thumbnail ??= {}).url = res.link;
					else if (input === imgLink) ((json ??= {}).image ??= {}).url = res.link;
					else ((json ??= {}).footer ??= {}).icon_url = res.link;
					update(json);
					console.info(
						`Image (${res.link}) will be deleted in 10 minutes. To delete it now, go to ${res.link.replace(
							'/files',
							'/del'
						)} and enter this code: ${res.authkey}`
					);
				})
	...
	update = data => {
		try {
			...
			if (data && Object.keys(data).length) {
							...
							`${
								emb.url
									? '<a class="anchor" target="_blank" href="' +
									  encodeHTML(url(emb.url)) +
									  '">' +
									  encodeHTML(emb.title) +
									  '</a>'
									: encodeHTML(emb.title)
							}`,
				...
				if (emb.description)
					display(embedDescription, markup(encodeHTML(emb.description), { inEmbed: true, replaceEmojis: true }));
				else hide(embedDescription);

				if (emb.color)
					embed.closest('.embed').style.borderColor =
				...
				if (emb.author?.name)
                    ...
                    ${
											emb.author.icon_url
												? '<img class="embedAuthorIcon" src="' + encodeHTML(url(emb.author.icon_url)) + '">'
												: ''
										}
                    ${
											emb.author.url
												? '<a class="embedAuthorNameLink embedLink embedAuthorName" href="' +
												  encodeHTML(url(emb.author.url)) +
												  '" target="_blank">' +
												  encodeHTML(emb.author.name) +
												  '</a>'
												: '<span class="embedAuthorName">' + encodeHTML(emb.author.name) + '</span>'
										}`,
						'flex'
					);
				...
				if (emb.thumbnail?.url) {
					(embedThumbnail.src = emb.thumbnail.url), (embedThumbnail.parentElement.style.display = 'block');
				...
				if (emb.image?.url) (embedImage.src = emb.image.url), (embedImage.parentElement.style.display = 'block');
				...
				if (emb.footer?.text)
                    ...
                    ${
											emb.footer.icon_url
												? '<img class="embedFooterIcon" src="' + encodeHTML(url(emb.footer.icon_url)) + '">'
												: ''
										}<span class="embedFooterText">
                        ${encodeHTML(emb.footer.text)}
                    ${
											emb.timestamp
												? '<span class="embedFooterSeparator">•</span>' + encodeHTML(tstamp(emb.timestamp))
												: ''
										}</span></div>`,
						'flex'
					);
				else if (emb.timestamp)
					display(
						embedFooter,
						`<span class="embedFooterText">${encodeHTML(tstamp(emb.timestamp))}</span></div>`,
						'flex'
					);
				...
				if (emb.fields) {
					...
					emb.fields.forEach((f, i) => {
						if (f.name && f.value) {
							...
							if (
								(emb.fields[i].inline &&
									emb.fields[i + 1]?.inline &&
									// it's the first field in the embed or -
									((i === 0 && emb.fields[i + 2] && !emb.fields[i + 2].inline) || // it's not the first field in the embed but the previous field is not inline or -
										(((i > 0 && !emb.fields[i - 1].inline) ||
											...
											(i >= 3 &&
												emb.fields[i - 1].inline &&
												emb.fields[i - 2].inline &&
												emb.fields[i - 3].inline &&
												(emb.fields[i - 4] ? !emb.fields[i - 4].inline : !emb.fields[i - 4]))) &&
											...
											(i == emb.fields.length - 2 || !emb.fields[i + 2].inline)))) ||
								(i % 3 === 0 && i == emb.fields.length - 2)
							) {
							...
							if (!f.inline)
								...
								fieldElement.outerHTML = `
                                    <div class="embedField ${num}${gridCol ? ' colNum-2' : ''}" style="grid-column: ${
									gridCol || colNum + ' / ' + (colNum + 4)
								};">
                                        <div class="embedFieldName">${markup(encodeHTML(f.name), {
																					inEmbed: true,
																					replaceEmojis: true,
				...
				for (block of document.querySelectorAll('.markup pre > code')) hljs.highlightBlock(block);
			...
			if (
				!embedCont.innerText &&
				!document.querySelector('.messageContent + .container .embedGrid > [style*=display] img')
			)
				document.body.classList.add('emptyEmbed');

			json = data.embed || data;
			if (autoUpdateURL) urlOptions({ set: ['data', jsonToBase64(json)] });
		} catch (e) {
			console.error(e);
			console.dir(e);
			error(e);
		...
		try {
			if (dataKeys.length && !jsonKeys.some(key => dataKeys.includes(key))) {
				...
				if (usedKeys.length > 2)
					return error(
						`'${usedKeys[0] + "', '" + usedKeys.slice(1, usedKeys.length - 1).join("', '")}', and '${
							usedKeys[usedKeys.length - 1]
						}' are invalid keys.`
					);
				return error(
					`'${
						usedKeys.length == 2
							? usedKeys[0] + "' and '" + usedKeys[usedKeys.length - 1] + "' are invalid keys."
							: usedKeys[0] + "' is an invalid key."
					}`
				);
			} else if (!validationError) error(false);
			update(jsonData);
		} catch (e) {
	...
	setTimeout(() => {
		picker.on('change', function (r, g, b, a) {
			...
			json && (json.color = parseInt(this.color(r, g, b).slice(1), 16));
	...
	document.querySelector('.opt.json').addEventListener('click', () => {
		...
		if (document.querySelector('section.low')) togglePicker(true);
	});
	...
	document.body.addEventListener('click', e => {
		if (
			...
			(e.target.classList.contains('top') && colrs.classList.contains('display'))
		)
			togglePicker();
	});
	...
	document.querySelector('.colrs .hex>div').addEventListener('input', e => {
		...
		if (inputValue.startsWith('#')) (e.target.value = inputValue.slice(1)), (inputValue = e.target.value);
		...
		json.color = parseInt(inputValue, 16);
		update(json);
	});
	...
	document.querySelector('.top-btn.copy').addEventListener('click', e => {
		...
		if (
			!navigator.clipboard
				?.writeText(JSON.stringify(JSON.parse(jsonData)))
				.then(next)
